...
_images_list_file=rke2-images-all.linux-${ARCH}.txt
...
do_download_verify_file() {
    _sha256sum_file=sha256sum-${ARCH}.txt
...
do_download_rke2() {
    _rke2_package_file=rke2.linux-${ARCH}.tar.gz
...
do_download_rke2_core_images() {
    _rke2_core_images=rke2-images-core.linux-${ARCH}.tar.gz
...
do_download_rke2_calico_images() {
    _rke2_calico_images=rke2-images-calico.linux-${ARCH}.tar.gz
...
do_download_rke2_cilium_images() {
    _rke2_cilium_images=rke2-images-cilium.linux-${ARCH}.tar.gz
